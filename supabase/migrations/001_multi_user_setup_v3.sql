-- =====================================================
-- SOLDUCCI MULTI-USER MIGRATION V3 (Zero Recursion)
-- Fase 1: Database Setup
-- =====================================================
-- FIXED: Eliminata OGNI ricorsione dalle policy
-- =====================================================
...
-- =====================================================
-- STEP 3: CREATE RLS POLICIES (ZERO RECURSION!)
-- =====================================================
...
-- GROUP_MEMBERS POLICIES (Direct access only - NO subqueries!)
...
-- Allow viewing ALL group_members (filtered by app logic)
-- This prevents recursion since we don't query group_members FROM group_members
CREATE POLICY "Users can view all group memberships"
  ON group_members FOR SELECT
  USING (true);

-- Allow inserting (controlled by app logic + FK constraints)
...
-- GROUPS POLICIES (Using direct group_members query - NO recursion)
...
-- Allow viewing all groups (will be filtered by JOIN in service)
...
-- GROUP_INVITES POLICIES (Simple direct checks)
DROP POLICY IF EXISTS "Users can view their invites" ON group_invites;
DROP POLICY IF EXISTS "Users can view all invites" ON group_invites;
...
-- Allow viewing invites (will filter by email in service)
...
-- EXPENSE_SPLITS POLICIES (Simple)
DROP POLICY IF EXISTS "Users can view their splits" ON expense_splits;
DROP POLICY IF EXISTS "Users can view all splits" ON expense_splits;
...
-- Allow viewing all splits (filtered by app)
CREATE POLICY "Users can view all splits"
  ON expense_splits FOR SELECT
  USING (true);

-- Allow creating splits (controlled by app logic)
...
-- EXPENSES POLICIES (Simple - NO recursion!)
...
-- Allow viewing all expenses (will be filtered by context in app)
...
-- MIGRATION COMPLETE ✅
-- =====================================================
-- V3: Policy completamente riscalate per eliminare OGNI ricorsione
-- Filtraggio spostato completamente a livello applicativo
-- =====================================================
