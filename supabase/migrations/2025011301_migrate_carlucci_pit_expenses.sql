...
DO $$
DECLARE
  target_group_id UUID := '1775bead-b76c-49c8-b8d7-620cba2758c4';
  pit_user_id UUID := '09ace514-a951-4936-afd6-468504075542';
  carlucci_user_id UUID := '821db3d9-f450-4c48-8844-1d649ab399ad';
...
BEGIN
  ...
  SET
    ...
    paid_by = CASE
      -- Carlucci paid
      WHEN LOWER(money_flow) IN ('carlucci', 'carlucci -> /2', 'carl -> pit') THEN carlucci_user_id::text

      -- Pit paid
      WHEN LOWER(money_flow) IN ('pit', 'pit -> /2', 'pit -> carl') THEN pit_user_id::text
  ...
  SELECT
    e.id,
    gm.user_id,
    ROUND(e.amount / 2.0, 2) as split_amount,
  ...
  SELECT
    ...
    ROUND(e.amount / 2.0, 2) as split_amount,  -- Each person owes half
