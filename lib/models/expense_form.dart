...
class ExpenseForm {
  ...
  final ExpenseField dateField = ExpenseField(fieldName: "Date", type: DateTime, value: DateTime.now());
  final ExpenseField typeField = ExpenseField(fieldName: "Tipologia", type: Tipologia);

  List<dynamic> getFieldsNames() => [descriptionField.getFieldName(), moneyField.getFieldName(), flowField.getFieldName(), dateField.getFieldName(), typeField.getFieldName()];
  List<dynamic> getFieldValues() => [descriptionField.getFieldValue(), moneyField.getFieldValue(), flowField.getFieldValue(), dateField.getFieldValue(), typeField.getFieldValue()];
  Map<String, dynamic> getFieldsMap() => {
    ...
    dateField.getFieldName(): DateFormat('dd/MM/yyyy').format(dateField.getFieldValue()),
        ...
        children: [
          Row(
            ...
            children: [
              ...
              Flexible(
                flex:2,
                child: FieldWidget(expenseField: moneyField),
              )
            ],
          ),
          Column(
            crossAxisAlignment: CrossAxisAlignment.center,
            children: [flowField, dateField, typeField].map((el) => FieldWidget(expenseField: el)).toList(),
          ),
          IconButton(
            onPressed: () async {
              if (_formKey.currentState!.validate()) {
                _formKey.currentState!.save();
                await SheetApi.insert(getFieldsMap());
...
Widget build(BuildContext context) {
  ...
  if (type == String)  {
    ...
    CurrencyTextInputFormatter formatter = CurrencyTextInputFormatter.currency(locale: 'it', symbol: '€');
    return Container(
      ...
      return Container(
        ...
        child: EnumFormField<Enum>( // Use Enum as the generic type or a more specific base if possible
          title: values[0].getTypeTitle(), // Access getTypeTitle from EnumLabel extension
          options: values,
          onChanged: (selected) {
            setState(() {
      ...
      return Container(
        ...
        child: Row(
          ...
          onPressed: () async {
            ...
            if (picked != null && picked != widget.expenseField.getFieldValue()) {
              setState(() {
...
enum MoneyFlow {
  carlToPit('Carlucci -> Pitucci'),
  pitToCarl('Pitucci -> Carlucci'),
  carlDiv2('Carlucci -> /2'),
  pitDiv2('Pitucci -> /2'),
  carlucci('Carlucci'),
  pit('Pitucci');
...
class EnumFormField<T extends Enum> extends FormField<T> {
  ...
  final ValueChanged<T?>? onChanged; // Callback for when a radio button is selected
  ...
  EnumFormField({
    ...
    this.onChanged,
    T? initialValue, // Make initialValue nullable
